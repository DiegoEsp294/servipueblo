...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
        <div class="flex gap-4 items-start">
            <img src="{{ $worker->photo_url }}"
                 alt="{{ $worker->name }}"
                 class="w-20 h-20 sm:w-24 sm:h-24 rounded-full object-cover bg-gray-100 shrink-0">
                ...
                <div class="mt-2 flex flex-wrap items-center gap-3">
                    ...
                    @if($worker->ratings_count > 0)
                        ...
                        <span class="text-sm font-medium text-green-700">
                            👍 {{ $worker->recommendations_count }} de {{ $worker->ratings_count }} lo recomiendan
                        </span>
                    @endif
        ...
        <div class="mt-5 flex gap-3 flex-wrap">
            ...
            <a href="{{ route('workers.whatsapp', $worker->slug) }}"
               class="inline-flex items-center gap-1.5 bg-[#25D366] hover:bg-[#1ebe5d] text-white font-medium rounded-full transition-colors text-base px-6 py-3">
            ...
            <a href="https://wa.me/?text={{ urlencode('Te comparto el perfil de ' . $worker->name . ' (' . $categoryNames . ') en ServiPueblo: ' . route('workers.show', $worker->slug)) }}"
               ...
               onclick="trackShare()"
               class="inline-flex items-center gap-2 bg-green-50 hover:bg-green-100 border border-green-300 text-green-700 text-sm font-medium px-4 py-2.5 rounded transition-colors">
            ...
            <button onclick="document.getElementById('qr-section').classList.toggle('hidden')"
                    class="inline-flex items-center gap-2 bg-gray-50 hover:bg-gray-100 border border-gray-300 text-gray-600 text-sm font-medium px-4 py-2.5 rounded transition-colors">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        @else
            ...
            {{-- Usuario no logueado --}}
            <p class="text-sm text-gray-500 mb-4">Para dejar una calificación iniciá sesión con tu cuenta de Google.</p>
            <a href="{{ route('auth.google', ['redirect' => route('workers.show', $worker->slug)]) }}"
               class="inline-flex items-center justify-center gap-3 w-full sm:w-auto bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 text-sm font-medium px-5 py-2.5 rounded shadow-sm transition-colors">
...
<script>
    ...
    function downloadQR() {
        ...
        ctx.fillText('{{ $worker->category->name }} · {{ $worker->town }}', cardWidth / 2, qrSize + 87);
...
<script>
    function trackShare() {
        fetch('{{ route('workers.share', $worker->slug) }}', {
            method: 'POST',
